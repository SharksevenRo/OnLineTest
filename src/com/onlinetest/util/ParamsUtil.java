...
public class ParamsUtil {
	 ...
	 * @throws Exception
	 */
	public static <T> PreparedStatement injectParams(PreparedStatement ps, T t) throws Exception {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			// 当不是time类型的字段时（因为时间字段是数据库timestamp类型）
			if (!fields[i].getName().endsWith("time")) {
		...
		while(rs.next()){
			...
			for (int i = 0; i < fields.length; i++) {
				...
				if (fieldName.endsWith("time")) {
					...
					Method method = clazz
							.getMethod("set" + fieldName, Timestamp.class);
